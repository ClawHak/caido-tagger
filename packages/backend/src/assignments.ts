...
export async function removeTagFromRequest(
  ...
  await db
    .prepare(`DELETE FROM request_tags WHERE request_id = ? AND tag_id = ?`)
...
export async function removeAllTagsFromRequest(
  ...
  await db
    .prepare(`DELETE FROM request_tags WHERE request_id = ? AND project_id = ?`)
...
export async function getTaggedRequestIds(
  ...
  if (tag_ids && tag_ids.length > 0) {
    // Multi-tag filter: requests that have ALL specified tags
  ...
  // All tagged requests in the project
  const rows = await db
    .prepare(
      `SELECT rt.request_id, rt.tagged_at,
              GROUP_CONCAT(rt.tag_id) as tag_ids_csv
